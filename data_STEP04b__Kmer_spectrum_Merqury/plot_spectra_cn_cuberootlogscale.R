...
cube_root_trans <- scales::trans_new(
  name = "cube_root",
...
plot_line <- function(dat, name, x_max, y_max, zero, cutoff) {
  ggplot(data=dat, aes(x=kmer_multiplicity, y=Count, group=dat[,1], colour=dat[,1])) +
    ...
    theme_bw() +
    format_theme() +
    scale_y_continuous(trans=cube_root_trans,labels=fancy_scientific) +
    coord_cartesian(xlim=c(0,x_max), ylim=c(0,y_max))
}


# CUSTOM PLOT FILL FUNCTION
plot_fill <- function(dat, name, x_max, y_max, zero, cutoff, k = 7) {
  # dat is assumed to have columns: (class in dat[,1]), kmer_multiplicity, Count

  # Rolling-mean smoother with edge handling
  smooth_vec <- function(v, k) {
    v_s <- stats::filter(v, rep(1 / k, k), sides = 2)
    v_s <- as.numeric(v_s)
    # Fill NA at edges with original values (minimal distortion)
    v_s[is.na(v_s)] <- v[is.na(v_s)]
    v_s
  }

  # Ensure deterministic ordering for smoothing (important)
  ord <- order(dat[, 1], dat$kmer_multiplicity)
  dat2 <- dat[ord, , drop = FALSE]

  # Smooth Count within each group (dat[,1] is the group key)
  grp <- dat2[, 1]
  dat2$Count_smooth <- ave(
    dat2$Count,
    grp,
    FUN = function(v) smooth_vec(v, k)
  )

  ggplot(data = dat2, aes(x = kmer_multiplicity, y = Count_smooth)) +
    geom_ribbon(
      aes(
        ymin = 0,
        ymax = pmax(Count_smooth, 0),
        fill = dat2[, 1],
        colour = dat2[, 1]
      ),
      alpha = ALPHA,
    ...
    theme_bw() +
    format_theme() +
    scale_color_manual(values = merqury_brw(dat2[, 1]), name = "k-mer") +
    scale_fill_manual(values = merqury_brw(dat2[, 1]), name = "k-mer") +
    scale_y_continuous(trans = cube_root_trans, labels = fancy_scientific) +
    coord_cartesian(xlim = c(0, x_max), ylim = c(0, y_max))
}
